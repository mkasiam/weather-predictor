...
const MonthlyWeather = () => {
  ...
  const getDailyAverages = () => {
    ...
    return Object.entries(days).map(([date, data]) => ({
      ...
      avgPrecipitation:
        data.precipitation.reduce((a, b) => a + b, 0) /
        data.precipitation.length,
  ...
  const monthlyStats = {
    ...
    avgHumidity:
      dailyAverages.reduce((sum, day) => sum + day.avgHumidity, 0) /
      dailyAverages.length,
    avgWind:
      dailyAverages.reduce((sum, day) => sum + day.avgWind, 0) /
      dailyAverages.length,
    rainyDays: dailyAverages.filter((day) => day.avgPrecipitation > 0.3).length,
    sunnyDays: dailyAverages.filter((day) => day.dominantWeather === "Clear")
      .length,
    cloudyDays: dailyAverages.filter((day) => day.dominantWeather === "Clouds")
      .length,
  };
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-red-50 rounded-lg p-4 text-center">
          ...
          <div className="text-xs text-gray-500 mt-1">
            High: {Math.round(monthlyStats.maxTemp)}° Low:{" "}
            {Math.round(monthlyStats.minTemp)}°
          </div>
        </div>
      ...
      <div className="mb-6">
        <h4 className="font-medium text-gray-800 mb-4">
          Daily Breakdown (Available Data)
        </h4>
              ...
              >
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {Math.round(day.avgPrecipitation * 100)}%
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
        <div className="text-sm text-yellow-800">
          <strong>Note:</strong> Monthly overview is based on available 5-day
          forecast data. For complete monthly historical data, a premium weather
          API subscription would be required.
        </div>
      </div>
    </div>
  );
